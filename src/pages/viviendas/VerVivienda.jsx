...
export default function VerVivienda() {
  ...
  useEffect(() => {
    if (vivienda) cargarRelacionados();
  }, [vivienda]);
    ...
    <Menu>
      <div style={{
        ...
        <div style={{
          ...
          <p><strong style={{ color: "#4db8ff" }}>Dirección:</strong> {vivienda.direccion}</p>
          <p><strong style={{ color: "#4db8ff" }}>Ciudad:</strong> {vivienda.ciudad}</p>
          <p><strong style={{ color: "#4db8ff" }}>Código Postal:</strong> {vivienda.codigo_postal}</p>
        </div>
        ...
        <Bloque titulo="Cliente propietario">
          {!cliente ? (
            ...
            <Item to={`/clientes/${cliente.id}`} titulo={`${cliente.nombre} — ${cliente.email}`} />
          )}
        </Bloque>
        ...
        <button onClick={eliminarVivienda} style={{
          marginTop: "10px",
          padding: "14px",
          width: "100%",
          background: "red",
